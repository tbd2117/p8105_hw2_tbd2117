...
pols = 
  ...
  mutate(month = substring(pols$month, 2, 3)) %>% 
  mutate(month = as.numeric(pols$month)) %>% 
  mutate(month = month.name[month])

pols = 
  mutate(pols, president = if_else(prez_gop == 1, "gop", "dem")) %>%
...
snp = 
  ...
  mutate(month = as.numeric(snp$month)) %>% 
  mutate(month = month.name[month]) %>% 
...
unemployment = 
  ...
  mutate(month = month.name[unemployment$month]) %>% 
  mutate(year = as.character(unemplyment$year))
```
...
The dataset has `r nrow(snp_pols_unemployment)` rows and `r ncol(snp_pols_unemployment)` columns.
```
